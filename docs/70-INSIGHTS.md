...
The MCP specification (2025-03-26) defines OAuth 2.1 auth with scopes, and `mcp-handler` v1.1.0 supports `requiredScopes` for handler-level gating. However, **per-client `tools/list` filtering is not in the spec or Auth0's MCP docs**. The key insight: `mcp-handler` creates a new `McpServer` per request, and the `initializeServer` callback runs inside `withAuthContext`'s `AsyncLocalStorage`. By creating our own ALS to bridge `allowedTools` from `withMcpAuth` into the callback, we filter tool registration per-request — so `tools/list` returns only tools the client is allowed to call. This is defense-in-depth: execution-layer enforcement (Layer 3) remains as a fallback for stale tool caches.
