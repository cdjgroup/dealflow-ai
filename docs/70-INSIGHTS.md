...
## 018 — Per-client MCP tool filtering requires overriding the SDK's ListTools handler (2026-04-05)

The MCP specification (2025-03-26) defines OAuth 2.1 auth with scopes, and `mcp-handler` v1.1.0 supports `requiredScopes` for handler-level gating. However, **per-client `tools/list` filtering is not in the spec or library**. The `createMcpHandler` callback runs once at server initialization — tool registration is static, not per-session. Our solution: (1) populate `AuthInfo.extra` with client metadata (allowedTools, rateLimit, trustTier) in `verifyMcpToken()`, (2) override the SDK's `ListToolsRequestSchema` handler via `server.server.setRequestHandler()` to filter the tool list based on the requesting client's scopes and allowlist. The override captures the original handler (which handles Zod → JSON Schema conversion) and filters its output. This ensures restricted clients cannot enumerate tools they aren't allowed to call — closing a privilege information disclosure gap.
