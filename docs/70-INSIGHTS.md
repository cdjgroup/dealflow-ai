...
The IETF draft `draft-klrc-aiagent-auth-01` ("AI Agent Authentication and Authorization", March 30, 2026, co-authored by an OpenAI engineer Nick Steele) defines three delegation patterns in Section 10.3: User Delegates Authorization (10.3.1), Agent Obtains Own Authorization (10.3.2), and Agents Accessed by Systems or Other Agents (10.3.3). Our three surfaces map to these: Chat (user-delegated — user is present, Auth0 session), Action Center (pre-authorized — agent acts on schedule with stored refresh token), MCP (agent-to-agent — external agents invoke tools via API key). The Surface Policy Registry is a concrete implementation of what the draft describes abstractly. Section 10.6 "Human in the Loop" explicitly names CIBA as the mechanism for step-up agent consent — exactly our pattern for write operations. Source: https://datatracker.ietf.org/doc/draft-klrc-aiagent-auth/

## 023 — Surface Policy Registry independently implements permit.io's tool authorization pattern (2026-04-05)

The `SURFACE_POLICIES` registry in `src/lib/surface-policy.ts` independently arrived at the same tool-level authorization pattern that permit.io recommends for their "Secure External Access" perimeter (https://www.permit.io/ai-access-control). Both systems treat "which tools can be accessed" as a first-class authorization decision: permit.io recommends tool whitelists per agent identity; we implement tool whitelists per surface and per MCP client. The key difference: permit.io is a dynamic ABAC engine with runtime policy changes via `permit.check()`; ours are static TypeScript declarations — no external dependency, no runtime call. The layered intersection pattern (surface policy ceiling → per-client policy narrows → user toggles narrow further) is portable: adding a new surface (e.g., "webhook") requires one registry entry, no tool changes. The comparison is accurate for permit.io's execution perimeter but does not extend to their other three perimeters (prompt filtering, RAG protection, response enforcement), which we don't implement.
...
## 029 — Stateless needsApproval creates approval loops in multi-step AI SDK flows (2026-04-05)

AI SDK v6's `needsApproval` is evaluated fresh on every tool-call round within a single `streamText` invocation. If the check is stateless (always returns `true` for a tool category), the model will re-propose the same tool after each execution, creating an infinite approve→execute→re-propose loop. The first fix attempt (wrapping `execute` to track approvals) failed because downstream wrappers (CIBA step-up, rate limiter) replace `execute`, so the tracking call never ran. The working fix: a shared `Set<string>` created at the request handler level, populated by `experimental_onToolCallFinish` (which fires regardless of wrapper order), and consumed by `needsApproval` to short-circuit. The insight: in a multi-wrapper tool pipeline, only SDK-level callbacks are guaranteed to run — wrapper-level instrumentation is fragile because any outer wrapper can replace the function.
